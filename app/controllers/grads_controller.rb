class GradsController < ApplicationController
  before_action :set_grad, only: [:show, :edit, :update, :destroy, :get_away, :come_back]

  def come_back
    @grad.update(active: true)
    redirect_to grads_path
  end

  def get_away
    @grad.update(active: false)
    redirect_to grads_path
  end
  ...
  private
    ...
    def grad_params
      params.require(:grad).permit(:name, :role, :photo)
    end
end
